...
int resizeDown(ArrayList* pList)
{
    ...
    if(pList != NULL)
    {
       if(pList->reservedSize < pList->reservedSize)
       {
          pAux = realloc(pList->pElements,-(sizeof(void*))*(AL_INCREMENT));

          if(pAux != NULL)
          {
             pList->pElements = pAux;
             pList->reservedSize= pList->reservedSize - pList->size;
          }
       }
...
int expand(ArrayList* pList,int index)
{
    ...
    if(pList->size == pList->reservedSize)
    {
        newSpace = (AL_INCREMENT+ pList->reservedSize) * sizeof(void*);
